 ...
 */
EditorUi.prototype.splitSize = (mxClient.IS_TOUCH || mxClient.IS_POINTER) ? 5 : 8;
 ...
 */
EditorUi.prototype.formatWidth = 250;
EditorUi.prototype.defaultFormatWidth = 250;
 ...
 * screen widths <= 640px.
 */
EditorUi.prototype.hsplitPosition = (screen.width <= 640) ? 118 : 250;
...
EditorUi.prototype.toggleFullscreen = function(forceHide)
{
	if(forceHide)
	{
		this.toggleSidebar(forceHide);
		this.toggleFormatPanel(forceHide);
		this.toggleMenuContainer(forceHide);
	}
	else
	{
		if(this.hsplitPosition == 0 && this.formatWidth == 0 && this.toolbarContainer.style.display === 'none')
		{
		...
		else
		{
			...
			this.toggleMenuContainer(true);						
		}
	}
}

EditorUi.prototype.toggleSidebar = function(forceHide)
{
	if(forceHide && this.hsplitPosition == 0) return;
	
	if(forceHide && this.hsplitPosition < 80)
	{
		this.hsplit.click();
		this.hsplit.click();		
	}	
...
EditorUi.prototype.toggleMenuContainer = function(forceHide)
{
	var currMenuStatus = (this.toolbarContainer.style.display === 'block' || this.toolbarContainer.style.display === '');
	
	if(forceHide || currMenuStatus)
	{
		this.toolbarContainer.style.display = 'none';
		this.formatContainer.classList.add('AgnityTopBorder');
		this.diagramContainer.classList.add('AgnityTopBorder');
	}
	else
	{
		this.toolbarContainer.style.display = 'block';
		this.formatContainer.classList.remove('AgnityTopBorder');
		this.diagramContainer.classList.remove('AgnityTopBorder');
	}
	
	this.refresh();
};

EditorUi.prototype.toggleAgnityNavigator = function(forceHide)
{
	if(this.format == null) return;

	if(forceHide || this.format.showOutline)
	{
		this.format.showOutline = false;
	}
	else
	{
		this.format.showOutline = true;
	}
...
{
	var user = Agnity.getLocalStorage('user');
	if (this.editor.modified && user != null)
	{
...
EditorUi.prototype.refresh = function(sizeDidChange)
{
	...
	if(effHsplitPosition < 75 && effHsplitPosition != 0)
	{
		effHsplitPosition = 75;	
	}
	
	var tmp = 0;
		
	if (this.toolbar != null || this.menubar != null)
	{
		this.toolbarContainer.style.top = '0px';
		this.toolbarContainer.style.left = (effHsplitPosition + this.splitSize) + 'px';
		tmp += this.toolbarContainer.clientHeight;
	}
	...
	if(effHsplitPosition < 225)
	{
		$(this.sidebarContainer).find(".AgnitySidebarPaletteLabel").each(function() {
			  $( this ).addClass( "AgnityHide" );
		});
	}
	else
	{
		$(this.sidebarContainer).find(".AgnitySidebarPaletteLabel").each(function() {
			  $( this ).removeClass( "AgnityHide" );
		});		
	}
	
	if(effHsplitPosition < 150)
	{
		$(this.sidebarContainer).find(".AgnitySidebarPaletteImage").each(function() {
			  $( this ).addClass( "AgnityHide" );
		});
		$(this.sidebarContainer).find(".agnity-closeToggleSidebar").each(function() {
			  $( this ).addClass( "AgnityHide" );
		});		
	}
	else
	...
	this.footerContainer.style.height = '0px';
	this.hsplit.style.top = this.sidebarContainer.style.top;
	this.hsplit.style.bottom = off + 'px';
...
EditorUi.prototype.createDivs = function()
{
	...
	this.toolbarContainer = this.createDiv('geToolbarContainer geMenubarContainer');
	this.sidebarContainer = this.createDiv('geSidebarContainer');
	this.formatContainer = this.createDiv('AgnityFormatContainer');
	...
	this.toolbarContainer.style.top = '0px';
	this.toolbarContainer.style.left = this.hsplitPosition + 'px';
	...
	this.formatContainer.style.width = '300px';
	
	//this.formatContainer.style.zIndex = '1';
	
	this.diagramContainer.style.right = '300px';
...
EditorUi.prototype.createUi = function()
{
	...
	{
		this.toolbarContainer.appendChild(this.menubar.container);
	}
...
{
	var ret = this.createDiv('geFooter');
	ret.innerHTML = "<center>Customized by <B>Shar Solutions LLC.</B></center>";
	return ret;
};
...
EditorUi.prototype.addSplitHandler = function(elt, horizontal, dx, onChange)
{
	...
	function dropHandler(evt)
	{
		...
		initial = null;
		start = null;
		setTimeout(function(){ ignoreClick = false; }, 500);
	...
	mxEvent.addListener(elt, 'click', function(evt)
	{
		if (!ignoreClick)
		{
			...
			console.log('Shar: Last:' + last + ' Next:' + next);
			if(next < 80 && last < 80)
			{
				next = 250;
				last = 0;
...
EditorUi.prototype.createKeyHandler = function(editor)
{
	...
	var agnityAltActions =
		{
			78 : this.actions.get('new'),
			79 : this.actions.get('open'),
			83 : this.actions.get('saveUiXml_int'),
			76 : this.actions.get('loadTreeDialog'),
			88 : this.actions.get('deleteDialog'),
			73 : this.actions.get('import'),
			69 : this.actions.get('export'),
			75 : this.actions.get('saveTemplateDialog'),
			67 : this.actions.get('loadTemplateDialog'),
			68 : this.actions.get('downloadServiceXml_int'),
			66 : this.actions.get('openServiceXmlDialog'),	
		}
	...
	mxKeyHandler.prototype.getFunction = function(evt)
	{
		if (graph.isEnabled())
		{
			if(mxEvent.isAltDown(evt) && !mxEvent.isShiftDown(evt) && !this.isControlDown(evt))
			{
				var action = agnityAltActions[evt.keyCode];
	...
	if (!this.editor.chromeless || this.editor.editable)
	{
		...
		//keyHandler.bindAction(78, true, 'new'); // Ctrl+N		
		//keyHandler.bindAction(83, true, 'open'); // Ctrl+S		
		keyHandler.bindAction(83, true, 'saveUiXml'); // Ctrl+S		
		keyHandler.bindAction(76, true, 'loadUiXml'); // Ctrl+L		
		//keyHandler.bindAction(83, true, 'import'); // Ctrl+S		
		//keyHandler.bindAction(83, true, 'export'); // Ctrl+S		
		//keyHandler.bindAction(83, true, 'validate'); // Ctrl+S		
		keyHandler.bindAction(83, true, 'saveTemplate', true); // Ctrl+Shift+S
		keyHandler.bindAction(76, true, 'loadTemplate', true); // Ctrl+Shift+L
		keyHandler.bindAction(68, true, 'downloadServiceXml'); // Ctrl+D		
