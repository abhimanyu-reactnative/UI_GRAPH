...
Format = function(editorUi, container)
{
	...
	this.agnityPropertyPanel = null;
	this.agnityOutlinePanel = null;
...
Format.prototype.init = function()
{
	...
	this.agnityPropertyPanel = Agnity.createPropertyPanel(this.editorUi);
	
	this.setLeftSpacing = mxUtils.bind(this, function(sender, evt)
	{
		var cell = graph.getSelectionCell();
		var state = graph.view.getState(cell);
		
		if(graph.isSelectionEmpty() || cell.edge) return;
		
		var currLeftSpacing = state.style['spacingLeft'];
		
		if(cell.geometry == null) return;
		var currWidth = cell.geometry.width;
						
		var expectedSpacing = Math.floor(45 * currWidth / 150);
		
		if(Math.abs(expectedSpacing - currLeftSpacing) < 2) return;
		...
		window.setTimeout(function () {
			...
			try
			{
				graph.setCellStyles('spacingLeft', expectedSpacing, [cell]);	
				ui.fireEvent(new mxEventObject('styleChanged', 'keys', ['spacingLeft'],
				                               'values', [expectedSpacing], 'cells', [cell]));
			}
			finally
			{
	...
	{
		console.log('SHAR: on update. ' + JSON.stringify(evt));
		this.clearSelectionState();
		this.refresh();
	});
	
	this.onSelectionUpdate = mxUtils.bind(this, function(sender, evt)
	{
		console.log('SHAR: on selection change' + JSON.stringify(evt));
		this.agnityPropertyPanel = Agnity.createPropertyPanel(this.editorUi);
		this.clearSelectionState();
		this.refresh();
	});
	
	this.onChangeUpdate =  mxUtils.bind(this, function(sender, evt)
    {
		var selectedCell = graph.getSelectionCell() || null;
		console.log('SHAR: on change update' + JSON.stringify(evt));

		if(selectedCell != null && selectedCell.edge)
		{
			Agnity.createPropertyPanel(this.editorUi);
		}
	...
	graph.getSelectionModel().addListener(mxEvent.CHANGE, this.onSelectionUpdate);
	graph.addListener(mxEvent.EDITING_STARTED, this.update);
	graph.addListener(mxEvent.EDITING_STOPPED, this.update);
	//graph.getModel().addListener(mxEvent.CHANGE, this.onChangeUpdate);
	//graph.getModel().addListener(mxEvent.CHANGE, this.setLeftSpacing);
	graph.getModel().addListener(mxEvent.CHANGE, this.update);

	graph.addListener(mxEvent.ROOT, mxUtils.bind(this, function()
	{
		console.log('On Root change');
		this.refresh();
	}));
...
Format.prototype.getPropertyPanelButtonsDiv = function(title, callback)
{
...
Format.prototype.getSaveCancelPanel = function()
{
	if(this.saveCancelPanel != null) return this.saveCancelPanel;
	
	this.saveCancelPanel = document.createElement('div');
	this.saveCancelPanel.setAttribute('class', 'AgnitySaveCancelPanel');
	
	var self = this;
	
	var saveButton = this.getPropertyPanelButtonsDiv(mxResources.get('save'), function()
	{
		Agnity.savePanelChanges(null);
	});
	
	mxEvent.addListener(saveButton, 'click', function(event){
		Agnity.savePanelChanges('AgnitySendCommand');
	});
	var cancelButton = this.getPropertyPanelButtonsDiv(mxResources.get('cancel'), function()
    {
		agnityGlobalData.prevSelectedCellData = null;
		self.agnityPropertyPanel = Agnity.createPropertyPanel(self.editorsUi);
		self.clearSelectionState();
		self.refresh();
    });
	var helpButton = this.getPropertyPanelButtonsDiv(mxResources.get('help'), function()
    {	
		if(Agnity.isComponentDiagram())
		{
			if(agnityGlobalData.prevSelectedCellData == null || agnityGlobalData.prevSelectedCellData instanceof AgnityComponentData)
			{
				self.editorUi.actions.get('helpLinkDialog').funct('COMPONENT');
			}
			else
			{
				self.editorUi.actions.get('helpLinkDialog').funct(Agnity.getNodeName(agnityGlobalData.prevSelectedCellData));
			}
		}
		else
		{
			if(agnityGlobalData.prevSelectedCellData == null || agnityGlobalData.prevSelectedCellData instanceof AgnityTreeData)
			{
				self.editorUi.actions.get('helpLinkDialog').funct('TREE');
			}		
			else
			{
				self.editorUi.actions.get('helpLinkDialog').funct(Agnity.getNodeName(agnityGlobalData.prevSelectedCellData));
	...
	saveButton.setAttribute('class', 'AgnityFormatPanelActionButtonClass');
	cancelButton.setAttribute('class', 'AgnityFormatPanelActionButtonClass AgnityFormatCancelButtonClass');
	helpButton.setAttribute('class', 'AgnityFormatPanelActionButtonClass AgnityFormatHelpButtonClass');
	
	if(Agnity.hasUrlReadonlyParam() || !Agnity.hasLockAcquired())
		saveButton.classList.add('AgnityDisableIcon');
	
	this.saveCancelPanel.appendChild(saveButton);
	this.saveCancelPanel.appendChild(cancelButton);
	this.saveCancelPanel.appendChild(helpButton);
	return this.saveCancelPanel;
}
...
Format.prototype.refresh = function()
{
	...
	var tabButtonsHolder = document.createElement('div');
	tabButtonsHolder.setAttribute('class', 'formatTabButtonHolder');
	
	var propertyTabButton = document.createElement('button');
	propertyTabButton.setAttribute('id', 'propertyTab');
	mxUtils.write(propertyTabButton, mxResources.get('properties'));
	tabButtonsHolder.appendChild(propertyTabButton);
	
	var styleTabButton = document.createElement('button');
	styleTabButton.setAttribute('id', 'lookAndFeelTab');
	mxUtils.write(styleTabButton, mxResources.get('layout'));
	tabButtonsHolder.appendChild(styleTabButton);
	
	var indicatorButtonHolder = document.createElement('div');
	indicatorButtonHolder.setAttribute('class', 'AgnityIndicatorButtonHolder');
	var indicatorButton = document.createElement('div');
	indicatorButton.setAttribute('class', 'AgnityIndicatorButton');
	indicatorButtonHolder.appendChild(indicatorButton);
	
	//tabButtonsHolder.appendChild(indicatorButtonHolder);
	
	var tabContentHolder = document.createElement('div');
	tabContentHolder.setAttribute('class', 'formatTabContentHolder');

	this.container.appendChild(tabButtonsHolder);
	this.container.appendChild(tabContentHolder);

	if(this.showOutline)
	{		
		var outlineContentHolder = document.createElement('div');
		outlineContentHolder.setAttribute('class', 'formatTabOutlineHolder');
	
		if(this.agnityOutlinePanel == null)
		{
			this.agnityOutlinePanel = Agnity.getOutlinePanel(ui);
		}	
	
		outlineContentHolder.appendChild(this.agnityOutlinePanel);
		this.container.appendChild(outlineContentHolder);
	}
	
	var propertyContentHolder = document.createElement('div');
	propertyContentHolder.appendChild(this.agnityPropertyPanel);
	propertyContentHolder.appendChild(this.getSaveCancelPanel());
	tabContentHolder.appendChild(propertyContentHolder);	
	
	var styleContentHolder = document.createElement('div');
	tabContentHolder.appendChild(styleContentHolder);
	
	if(Format.lastActiveTab == 'LookAndFeelTab')
	{
		styleTabButton.setAttribute('class', 'active');
		styleContentHolder.style.display = 'block';
		propertyContentHolder.style.display = 'none';	
		indicatorButton.style.marginLeft = '75%';
	}
	else
	{
		propertyTabButton.setAttribute('class', 'active');
		propertyContentHolder.style.display = 'block';		
		styleContentHolder.style.display = 'none';
		indicatorButton.style.marginLeft = '25%';
	}

	mxEvent.addListener(propertyTabButton, 'click', function(event)
	{
		Format.lastActiveTab = 'PropertyTab';
		propertyTabButton.setAttribute('class', 'active');
		propertyContentHolder.style.display = 'block';			
		styleTabButton.removeAttribute('class');
		styleContentHolder.style.display = 'none';
		indicatorButton.style.marginLeft = '25%';
	});
	...
	{
		mxEvent.addListener(styleTabButton, 'click', function(event)
    	{
    		Format.lastActiveTab = 'LookAndFeelTab';
    		styleTabButton.setAttribute('class', 'active');
    		styleContentHolder.style.display = 'block';
    		propertyTabButton.removeAttribute('class');
    		propertyContentHolder.style.display = 'none';			
    		indicatorButton.style.marginLeft = '75%';
    	});
	}
	...
	label.setAttribute('class', 'labelDiv');
	...
	if (graph.isSelectionEmpty())
	{
		...
		styleContentHolder.appendChild(label);
		this.panels.push(new DiagramFormatPanel(this, ui, styleContentHolder));
	}
	else if (graph.isEditing())
	{
		mxUtils.write(label, mxResources.get('text'));
		styleContentHolder.appendChild(label);
		this.panels.push(new TextFormatPanel(this, ui, styleContentHolder));
	}
	else
	{
		...
		var addClickHandler = mxUtils.bind(this, function(elt, panel, index, panelName)
		{
			var clickHandler = mxUtils.bind(this, function(evt)
			{
				Format.activeLookAndFeel = panelName;				
				...
				if (currentLabel != elt)
				{
					...
					{
						currentLabel.removeAttribute('class')
					}
	
					currentLabel = elt;
					currentLabel.setAttribute('class', 'active')
		...
		var styleTabButtonHolder = document.createElement('div');
		styleTabButtonHolder.setAttribute('class', 'StyleTabButtonHolder');
		styleContentHolder.appendChild(styleTabButtonHolder);
		
		var styleTabContentHolder = document.createElement('div');
		styleTabContentHolder.setAttribute('class', 'StyleTabContentHolder');
		styleContentHolder.appendChild(styleTabContentHolder);
		
		var idx = 0;

		label.style.width = (containsLabel) ? '50%' : '33%';
		...
		if (containsLabel)
		{
			styleTabButtonHolder.appendChild(label);
			styleTabButtonHolder.appendChild(label3);
		}
		else
		{
			styleTabButtonHolder.appendChild(label);
			styleTabButtonHolder.appendChild(label2);
			styleTabButtonHolder.appendChild(label3);

			mxUtils.write(label, mxResources.get('style'));
			
			stylePanel.style.display = 'none';
			stylePanel.setAttribute('id', 'stylePanel');
			this.panels.push(new StyleFormatPanel(this, ui, stylePanel));
			styleTabContentHolder.appendChild(stylePanel);

			addClickHandler(label, stylePanel, idx++, 'Style');
		}
		...
		styleTabContentHolder.appendChild(textPanel);
		
		// Arrange
		mxUtils.write(label3, mxResources.get('arrange'));

		var arrangePanel = document.createElement('div');
		arrangePanel.style.display = 'none';
		arrangePanel.setAttribute('id', 'arrangePanel');
		this.panels.push(new ArrangePanel(this, ui, arrangePanel));
		styleTabContentHolder.appendChild(arrangePanel);
		
		addClickHandler(label2, textPanel, idx++, 'Text');
		addClickHandler(label3, arrangePanel, idx++, 'Arrange');
		
		if(Format.activeLookAndFeel == 'Style')
		{
			stylePanel.style.display = 'block';
		}
		else if(Format.activeLookAndFeel == 'Text')
		{
			textPanel.style.display = 'block';
		}
		else if(Format.activeLookAndFeel == 'Arrange')
		{
...
/**
 * Adds the local var button to property
 */
BaseFormatPanel.prototype.displaySetLocalVar = function(inLabel)
{
	var localVarDiv = document.createElement('div');
	localVarDiv.setAttribute('id', 'localVarDiv');
	
	var localVarBtn = null;
	localVarBtn = mxUtils.button(mxResources.get('setLocalVar'), mxUtils.bind(this, function(evt)
	{
		this.editorUi.actions.get('setLocalVar').funct();
	}));
	localVarBtn.setAttribute('title', mxResources.get('setLocalVar'));
	localVarBtn.setAttribute('class', 'geBtn gePrimaryBtn AgnityFormatLayoutTabButton');
	localVarBtn.setAttribute('id', 'setLocalVarBtn');
	localVarDiv.appendChild(localVarBtn);
...
ArrangePanel.prototype.addLayerOps = function(div)
{
	...
	btn.setAttribute('class', 'AgnityFormatLayoutTabButton');
...
ArrangePanel.prototype.addGroupOps = function(div)
{
	...
	if (graph.getSelectionCount() == 1 && Agnity.hasUrlDebugParam())
	{
		console.log('[SHAR] Showing Edit Data in Format Panel');
		console.log('[SHAR] Has URL debug Param: ' + Agnity.hasUrlDebugParam());
...
StyleFormatPanel.prototype.addEditOps = function(div)
{
	...
	if (ss.image)
	{
		...
		btn2.setAttribute('class', 'AgnityFormatLayoutTabButton');
