...
// see
// https://code.google.com/p/google-caja/issues/detail?can=2&q=&colspec=ID%20Type%20Status%20Priority%20Owner%20Summary&groupby=&sort=&id=1296
...
Graph = function(container, model, renderHint, stylesheet, themes)
{
	...
	if (this.edgeMode)
	{
		...
		// Uses this event to process mouseDown to check the selection state
		...
		this.addListener(mxEvent.FIRE_MOUSE_EVENT, mxUtils.bind(this, function(sender, evt)
		{
			if (evt.getProperty('eventName') == 'mouseDown' && this.isEnabled())
			{
				...
				if (!mxEvent.isControlDown(me.getEvent()) && !mxEvent.isShiftDown(me.getEvent()))
		    	{
			    	...
			    	if (state != null)
			    	{
			    		// Checks if state was removed in call to stopEditing
						// above
		    ...
		    mouseMove: mxUtils.bind(this, function(sender, me)
		    {
		    		...
		    		!mxEvent.isShiftDown(me.getEvent()) && !mxEvent.isAltDown(me.getEvent()))
		    	{
			    	...
			    	if (start.point != null && start.state != null && start.event != null)
			    	{
			    			...
			    			Math.abs(start.point.y - me.getGraphY()) > tol)
			    		{
			    			...
			    			if (handler != null && handler.bends != null && handler.bends.length > 0)
			    			{
			    				...
			    				// Handles special case where label was clicked
								// on unselected edge in which
			    				// case the label will be moved regardless of
								// the handle that is returned
	    						...
	    						{
				    				// Source or target handle or connected for
									// direct handle access or orthogonal line
				    				// with just two points where the central
									// handle is moved regardless of mouse
									// position
				    					...
				    					handle == handler.bends.length - 1 || state.visibleTargetState != null)
				    				{
				    					if (!entity && handle != mxEvent.LABEL_HANDLE)
				    					{
					    					...
					    					// Default case where handles are at
											// corner points handles
					    					// drag of corner as drag of
											// existing point
					    					if (pts != null && ((edgeStyle == null && handle == null) ||
					    						edgeStyle == mxEdgeStyle.OrthConnector))
					    					{
					    						// Does not use handles if they
						    				...
						    				// Creates a new waypoint and starts
											// moving it
						    				if (handle == null)
						    				{
				    					...
				    					// Removes preview rectangle in graph
										// handler
	    						...
	    						{
	    							// Disables moves on entity to make it
									// consistent
			    					this.graphHandler.reset();
	    							me.consume();
	    						}
			    	...
			    	else
			    	{
				    	...
				    	if (state != null)
				    	{
				    		// Checks if state was removed in call to
							// stopEditing above
				    		if (this.model.isEdge(state.cell))
				    		{
				    			var cursor = null;
			    				...
			    				if (pts != null)
			    				{
			    					...
			    					{
		    							// Moving is not allowed for entity
										// relation but still indicate hover
										// state
	...
	if (typeof mxVertexHandler !== 'undefined')
	{
		...
		// Handles parts of cells by checking if part=1 is in the style and
		// returning the parent
		// if the parent is not already in the list of cells. container style is
		// used to disable
		// step into swimlanes and dropTarget style is used to disable acting as
		// a drop target.
	    ...
	    // Activates outline connect after 1500ms with touch event or if alt is
		// pressed inside the shape
...
/**
 * Specifies if the touch UI should be used (cannot detect touch in FF so always
 * on for Windows/Linux)
 */
...
/**
 * Sets the policy for links. Possible values are "self" to replace any
 * framesets, "blank" to load the URL in <linkTarget> and "auto" (default).
 */
...
/**
 * Scrollbars are enabled on non-touch devices (not including Firefox because
 * touch events cannot be detected in Firefox, see above).
 */
...
/**
 * Specifies if the app should run in chromeless mode. Default is false. This
 * default is only used if the contructor argument is null.
 */
 ...
 * scrollbars but no visible background page. A good value is large enough to
 * reduce the number of repaints that is caused for auto- translation, which
 * depends on this value, and small enough to give a small empty buffer around
 * the graph. Default is 400x400.
 */
...
Graph.prototype.labelLinkClicked = function(state, elt, evt)
{
	...
	if (href != null && !this.isPageLink(href))
	{
		if (!this.isEnabled())
		{
			...
			{
				// Avoids page reload for anchors (workaround for IE but used
				// everywhere)
...
/**
 * Returns true if the fiven href references an external protocol that should
 * never open in a new window. Default returns true for mailto.
 */
...
/**
 * Returns a rectangle describing the position and count of the background
 * pages, where x and y are the position of the top, left page and width and
 * height are the vertical and horizontal page count.
 */
	...
	while (match = this.placeholderPattern.exec(str))
	{
		...
		if (val.length > 2 && val != '%label%' && val != '%tooltip%')
		{
			...
			else
			{
				...
				// Workaround for invalid char for getting attribute in older
				// versions of IE
				if (name.indexOf('{') < 0)
				{
					var current = cell;
...
// var elbowValue = (direction == mxConstants.DIRECTION_NORTH || direction ==
// mxConstants.DIRECTION_SOUTH) ? 'vertical' : 'horizontal';
				...
				for (var i = 0; i < cells.length; i++)
				{
					...
					if (state != null && geo != null)
					{
						...
						if (dy != 0 || dx != 0)
						{
							...
							if (layout == null)
							{
								// Moves cells to the right and down after
								// collapse/expand
...
Graph.prototype.resizeParentStacks = function(parent, layout, dx, dy)
{
	if (this.layoutManager != null && layout != null && layout.constructor == mxStackLayout && !layout.resizeLast)
	{
		...
		try
		{
			...
			// Bubble resize up for all parent stack layouts with same
			// orientation
 ...
 * Parameters:
 * 
 * parent - Optional <mxCell> whose children should be selected. Default is
 * <defaultParent>.
 */
 ...
 * Parameters:
 * 
 * vertices - Boolean indicating if vertices should be selected. edges - Boolean
 * indicating if edges should be selected. parent - Optional <mxCell> that acts
 * as the root of the recursion. Default is <defaultParent>.
 */
 ...
 * 
 * Returns the bottom-most swimlane that intersects the given point (x, y) in
 * the cell hierarchy that starts at the given parent.
 * 
 * Parameters:
 * 
 * x - X-coordinate of the location to be checked. y - Y-coordinate of the
 * location to be checked. parent - <mxCell> that should be used as the root of
 * the recursion. Default is <defaultParent>.
 */
...
Graph.prototype.getTooltipForCell = function(cell)
{
	...
	{
		var tmp = cell.value.getAttribute('errorMsg');
		...
		else
		{
			...
			if(!Agnity.hasUrlDebugParam())
			{
				ignored.push('agnityData')
			}
...
/**
 * Removes all illegal control characters with ASCII code <32 except TAB, LF and
 * CR.
 */
...
/**
 * Whether to hide arrows that collide with vertices. LATER: Add keyboard
...
HoverIcons.prototype.init = function()
{
	...
	this.graph.addMouseListener(
	{
	    ...
	    mouseUp: mxUtils.bind(this, function(sender, me)
	    {
	    	...
	    	{
	    		// Selects target vertex after drag and clone if not only new
				// edge was inserted
...
// es.cell.style = mxUtils.setStyle(es.cell.style, 'edgeStyle',
// es.style['edgeStyle']);
	...
	if (this.currentState != null)
	{
			...
			this.graph.isCellConnectable(this.currentState.cell))
		{
			...
			if (this.checkCollisions)
			{
				...
				// Shows hover icons large cell is behind all directions of
				// current cell
				...
				var checkCollision = mxUtils.bind(this, function(cell, arrow)
				{
					...
					// Ignores collision if vertex is more than 3 times the size
					// of this vertex
...
(function()
{
				...
				mxUtils.getValue(state.style, 'jumpStyle', 'none') !== 'none')
			{
				...
				for (var i = 0; i < pts.length - 1; i++)
				{
					...
					for (var e = 0; e < this.validEdges.length; e++)
					{
						...
						{
							// Compares each segment of the edge with the
							// current segment
							for (var j = 0; j < pts2.length - 1; j++)
							{
									...
									Math.abs(pt.y - p3.y) > thresh))
								{
									...
									// Intersections must be ordered by distance
									...
									// Ignores multiple intersections at segment
									// joint
	...
	/**
	 * Adds custom stencils defined via shape=stencil(value) style. The value is
	 * a base64 encoded, compressed and URL encoded XML definition of the shape
	 * according to the stencil definition language of mxGraph.
	 * 
	 * Needs to be in this file to make sure its part of the embed client code.
	 * Also the check for ZLib is different than for the Editor code.
	 */
	...
	mxCellRenderer.prototype.createShape = function(state)
	{
		if (state.style != null && typeof(pako) !== 'undefined')
		{
	    	...
	    	// Extracts and decodes stencil XML if shape has the form
			// shape=stencil(value)
 ...
 */
/**
 * Maps from library names to an array of Javascript filenames, which are
 * synchronously loaded. Currently only stencil files (.xml) and JS files (.js)
 * are supported. IMPORTANT: For embedded diagrams to work entries must also be
 * added in EmbedServlet.java.
 */
...
/**
 * Stores all package names that have been dynamically loaded. Each package is
 * only loaded once.
 */
			...
			if (libs != null)
			{
				if (mxStencilRegistry.packages[basename] == null)
				{
					for (var i = 0; i < libs.length; i++)
					{
						...
						{
							// FIXME: This does not yet work as the loading is
							// triggered after
							// the shape was used in the graph, at which point
							// the keys have
...
/**
 * These overrides are only added if mxVertexHandler is defined (ie. not in
 * embedded graph)
 */
if (typeof mxVertexHandler != 'undefined')
{
	(function()
	{
		...
		// Extends connection handler to enable ctrl+drag for cloning source
		// cell
		...
		// Overrides connection handler to ignore edges instead of not allowing
		// connections
		...
		Graph.prototype.importGraphModel = function(node, dx, dy, crop)
		{
			...
			try
			{
				// Mapping for multiple calls to cloneCells with the same set of
				// cells
				...
				for (var i = 0; i < childCount; i++)
				{
					...
					{
						// Delta is non cascading, needs separate move for
						// layers
		...
		Graph.prototype.getAllConnectionConstraints = function(terminal, source)
		{
			if (terminal != null)
			{
				...
				if (constraints != null)
				{
					// Requires an array of arrays with x, y (0..1) and an
					// optional
		...
		/**
		 * Updates the child cells with placeholders if metadata of a cell has
		 * changed.
		 */
		...
		/**
		 * Overrides double click handling to avoid accidental inserts of new
		 * labels in dblClick below.
		 */
		...
		/**
		 * Overrides double click handling to add the tolerance and inserting
		 * text.
		 */
		...
		/**
		 * Hook for subclassers to return true if the current insert point was
		 * defined using a mouse hover event.
		 */
		...
		/**
		 * Adds a new label at the given position and returns the new cell.
		 * State is an optional edge state to be used as the parent for the
		 * label. Vertices are not allowed currently as states.
		 */
		Graph.prototype.addText = function(x, y, state)
		{
			...
			if (state != null)
			{
				...
				// Resets the offset inside the geometry to find the offset from
				// the resulting point
		...
		/**
		 * Adds a handler for clicking on shapes with links. This replaces all
		 * links in labels.
		 */
			    ...
			    mouseUp: function(sender, me)
			    {
			    		...
			    		mxEvent.isTouchEvent(evt))))
			    	{
				    	if (this.currentLink != null) 
				    	{
							...
							{
								// Avoids page reload for anchors (workaround
								// for IE but used everywhere)
		...
		Graph.prototype.duplicateCells = function(cells, append)
		{
			...
			try
			{
				...
				for (var i = 0; i < cells.length; i++)
				{
					...
					{
						// Maintains child index by inserting after cloned in
						// parent
		...
		/**
		 * Inserts the given image at the cursor in a content editable text box
		 * using the insertimage command on the document instance.
		 */
		Graph.prototype.insertImage = function(newValue, w, h)
		{
			// To find the new image, we create a list of all existing links
			// first
			if (newValue != null)
			{
				...
				// LATER: Fix inserting link/image in IE8/quirks after focus
				// lost
				...
				if (newImages.length == oldImages.length + 1)
				{
					...
					for (var i = newImages.length - 1; i >= 0; i--)
					{
						...
						{
							// Workaround for lost styles during undo and redo
							// is using attributes
		 ...
		 * 
		 * Distribuets the centers of the given cells equally along the
		 * available horizontal or vertical space.
		 * 
		 * Parameters:
		 * 
		 * horizontal - Boolean that specifies the direction of the
		 * distribution. cells - Optional array of <mxCells> to be distributed.
		 * Edges are ignored.
		 */
		...
		Graph.prototype.getSvg = function(background, scale, border, nocrop, crisp, ignoreSelection, showText)
		{
			...
			{
				// KNOWN: Ignored in IE9-11, adds namespace for each image
				// element instead. No workaround.
		    ...
		    // Renders graph. Offset will be multiplied with state's scale when
			// painting state.
			// TextOffset only seems to affect FF output but used everywhere for
			// consistency.
			...
			// Adds simple text fallback for viewers with no support for
			// foreignObjects
		...
		/**
		 * Returns the first ancestor of the current selection with the given
		 * name.
		 */
		...
		Graph.prototype.pasteHtmlAtCaret = function(html)
		{
		    ...
		    if (window.getSelection)
		    {
		        ...
		        if (sel.getRangeAt && sel.rangeCount)
		        {
		            ...
		            // Range.createContextualFragment() would be useful here but
					// is
		            // only relatively recently standardized and is not
					// supported in
		...
		/**
		 * Creates an anchor elements for handling the given link in the hint
		 * that is shown when the cell is selected.
		 */
		...
		Graph.prototype.initTouch = function()
		{
			...
			// Context menu trigger implementation depending on current
			// selection state
			...
			// Shows popup menu if cell was selected or selection was empty and
			// background was clicked
			// FIXME: Conflicts with
			// mxPopupMenuHandler.prototype.getCellForPopupEvent in Editor.js by
			// selecting parent for selected children in groups before this
			// check can be made.
		...
		/**
		 * Handling of special nl2Br style for not converting newlines to breaks
		 * in HTML labels. NOTE: Since it's easier to set this when the label is
		 * created we assume that it does not change during the lifetime of the
		 * mxText instance.
		 */
		...
		mxCellEditor.prototype.startEditing = function(cell, trigger)
		{
			...
			// Stores current selection range when switching between markup and
			// code
		...
		mxCellEditor.prototype.installListeners = function(elt)
		{
			...
			// Handles paste from Word, Excel etc by removing styles, classnames
			// and unused nodes
		...
		mxCellEditor.prototype.toggleViewMode = function()
		{
			...
			if (!this.codeViewMode)
			{
			    ...
			    // Workaround for trailing line breaks being ignored in the
				// editor
		...
		/**
		 * Returns the background color to be used for the editing box. This
		 * returns the label background for edge labels and null for all other
		 * cases.
		 */
		...
		/**
		 * Defines the handles for the UI. Uses data-URIs to speed-up loading
		...
		// Enables connections along the outline, virtual waypoints, parent
		// highlight etc
		...
		if (Graph.touchStyle)
		{
			...
			if (mxClient.IS_TOUCH || navigator.maxTouchPoints > 0 || navigator.msMaxTouchPoints > 0)
			{
				...
				// Implements a smaller tolerance for mouse events and a larger
				// tolerance for touch
				// events on touch devices. The default tolerance (4px) is used
				// for mouse events.
		...
		// Overrides/extends rubberband for space handling with Ctrl+Shift(+Alt)
		// drag ("scissors tool")
		...
		mxRubberband.prototype.mouseMove = function(sender, me)
		{
			if (!me.isConsumed() && this.first != null)
			{
				...
				if (this.div != null || Math.abs(dx) > tol ||  Math.abs(dy) > tol)
				{
					...
					// Clears selection while rubberbanding. This is required
					// because
		...
		// Special case for single edge label handle moving in which case the
		// text bounding box is used
		...
		// Redirects moving of edge labels to mxGraphHandler by not starting
		// here.
		...
		// Workaround for "isConsumed not defined" in MS Edge is to use
		// arguments
