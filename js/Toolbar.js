...
Toolbar.prototype.init = function()
{
	...
	//this.addItems(['validate']);
	
	{
		var elts = this.addItems(['lockComponent', 'unLockComponent']);
		elts[0].setAttribute('title', mxResources.get('lock'));
		elts[0].style.paddingTop = '5px';
		elts[0].setAttribute('id', mxResources.get('lock'));
		elts[1].setAttribute('title', mxResources.get('unlock'));
		elts[1].setAttribute('id', mxResources.get('unlock'));
		elts[1].style.paddingTop = '5px';
	}
	...
	if (sw >= 400)
	{
		...
		this.edgeStyleMenu = this.addMenuFunction('geSprite-curved', mxResources.get('waypoints'), false, mxUtils.bind(this, function(menu)
		{
		...
		this.addDropDownArrow(this.edgeStyleMenu, 'geSprite-curved', 44, 50, 0, 0, 22, -4);
	}
	...
	//this.addItems(['variableManagement']);
	//this.addItems(['validate', 'variableManagement']);
};
...
{
	var elt = (showLabels) ? this.createLabel(label) : this.createButton(label, tooltip);
...
Toolbar.prototype.addItem = function(sprite, key, c, ignoreDisabled)
{
	...
	if (action != null)
	{
		...
		if(key == 'validate')
		{
			elt = this.addValidateButton(sprite, tooltip, action.funct, c);
		}
		else if(key == 'variableManagement')
		{
			elt = this.addvariableManagementBtn(sprite, tooltip, action.funct, c);
		}
		else
		{
...
/**
 * Adds a validate button to the toolbar
 */
Toolbar.prototype.addValidateButton = function(className, tooltip, funct, c)
{
	var elt = this.createButton(className, '');
	elt.setAttribute('id', 'AgnityValidateBtn');
...
/**
 * Adds a variable management button to the toolbal
 */
Toolbar.prototype.addvariableManagementBtn = function(className, tooltip, funct, c)
{
	var elt = this.createButton(className, '');
	elt.setAttribute('id', 'AgnityVariableMgntBtn');
...
{
	var elt = this.createButton(classname, tooltip);
	c = (c != null) ? c : this.container;
 ...
 */
Toolbar.prototype.createButton = function(classname, tooltip)
{
	...
	if(tooltip == 'Validate')
	{
		var t = document.createTextNode(mxResources.get('validate'));
		inner.appendChild(t);
	}
	else if(tooltip == 'Variable Management')
	{
		var t = document.createTextNode(mxResources.get('variableManagement'));
		inner.appendChild(t);
	}
