...
Sidebar.prototype.init = function()
{
	...
	var iconDir = IMAGE_PATH;
	var self = this;

	var actionPalette = ['variableManagement', 'validate', 'closeToggleSidebar'];
	
	var componentActionPalette = ['validate', 'closeToggleSidebar'];
	var componentGeneralAssetShapes = ['Function Block', 'Tree Node'];
	
	var dbSchemaGeneralAssetShapes = ['Table', 'Sequence', 'View', 'User Type'];
	
	var processGeneralAssetsShapes = ['Process Start', 'Return', 'Assignment', 'DB Call', 'Condition', 'Expression', 'Connect', 'External Call', 'Switch', 'Process Call',
	 'Application Measurement Counter','Http Ra', 'Logging'];
	var processCallAssetsShapes = ['Event Handler', 'Create Timer', 'Stop Timer', 'Call Hold', 'Resync Call', 'Dialout Call', 'Route Call', 'Terminate', 'End Execution', 'SIP Header', 'SMPP', 'CCB', 'ATI','UDR', 'Service Chaining' , 'checkpoint', 'Force Call Cleanup','GDI','DISCONNECT LEG', 'Call Queuing','Initiate SS7 Call'];
	var processMediaAssetsShapes = ['Play and Collect', 'Play', 'Create Conference', 'Destroy Conference', 'Record', 'Stop Media', 'Send Email', 'Send SMS', 'PlaySpeech', 'Join Conference', 'UnJoin Conference'];
	var processEnterpriseAssetsShapes = ['Time Pattern', 'Origin Pattern','Tui', 'Account Code', 'Block Pattern', 'Call Area Screening', 'Dialed Pattern', 'Percentage', 'Digit Pattern', 'Send Alarm', 'Echo Data'];
	
	var treeGeneralAssetsShapes = ['Start', 'Save CDR','Interm CDR' ,'Assignment', 'DB Call', 'Condition', 'Expression', 'Connect', 'External Call', 'Switch', 'Process Call','Peg Count',
	 'Application Measurement Counter','Http Ra', 'Logging'];
	//var treeGeneralAssetsShapes = ['Start', 'Save CDR','Interm CDR' ,'Assignment', 'DB Call', 'Condition', 'Expression', 'Connect', 'External Call', 'Switch', 'Process Call','Peg Count', 'Application Measurement Counter'];
	
	var treeCallAssetsShapes = ['Event Handler', 'Create Timer', 'Stop Timer', 'Call Hold', 'Resync Call', 'Dialout Call', 'Route Call', 'Terminate', 'End Execution', 'Automatic Code Gapping', 'SIP Header', 'SMPP', 'CCB', 'ATI','UDR', 'Service Chaining' , 'checkpoint', 'Force Call Cleanup','GDI','DISCONNECT LEG', 'Call Queuing','Initiate SS7 Call'];
	var treeMediaAssetsShapes = ['Play and Collect', 'Play', 'Create Conference', 'Destroy Conference', 'Record', 'Stop Media', 'Send Email', 'Send SMS', 'PlaySpeech', 'Join Conference', 'UnJoin Conference'];
	var treeEnterpriseAssetsShapes = ['Time Pattern', 'Origin Pattern', 'Account Code', 'Block Pattern', 'Call Area Screening', 'Dialed Pattern', 'Percentage', 'Digit Pattern', 'Send Alarm', 'Apply Charge', 'Routing Engine',
	 'Route Play', 'Route Play Collect', 'Route', 'Trigger Rule', 'Tui', 'Enum', 'Avp Reader', 'Avp Writer', 'Diameter Ccr', 'Diameter Cca','Echo Data'];	
	
	var dataProvider = new AgnityGetLicensedShapesHelper(Agnity.getUrlParam('diagram'), Agnity.hasUrlReadonlyParam(), Agnity.getUrlParam('operationMode'), Agnity.getDomainName());
	dataProvider.fetchData(function(response) {
		var inShapes = response.toString().split(',');
		for(var idx = 0; idx < inShapes.length; idx++)
		{
			self.licensedShapes.push(inShapes[idx]);
		}

		if(Agnity.isComponentDiagram())
		{		
			for(var idx = 0; idx < componentGeneralAssetShapes.length; idx++)
			{
				self.licensedShapes.push(componentGeneralAssetShapes[idx]);
			}
			self.addAgnityActions(false, iconDir, 'action', componentActionPalette);
			//self.addSearchPalette(true);
			self.addAgnityShapesPalette(true, dir, 'generalAssets', componentGeneralAssetShapes);	
		}
		else if(Agnity.isProcessDiagram())
		{
			self.addAgnityActions(false, iconDir, 'action', actionPalette);
			//self.addSearchPalette(true);
			if(!Agnity.hasUrlReadonlyParam() || Agnity.hasLockAcquired())
				self.addSearchPalette(true);
			self.addAgnityShapesPalette(true, dir, 'generalAssets', processGeneralAssetsShapes);
			self.addAgnityShapesPalette(true, dir, 'callAssets', processCallAssetsShapes);
			self.addAgnityShapesPalette(true, dir, 'mediaAssets', processMediaAssetsShapes);
			self.addAgnityShapesPalette(true, dir, 'enterpriseAssets', processEnterpriseAssetsShapes);
		}
		else if(Agnity.isTreeDiagram())
		{
			self.addAgnityActions(false, iconDir, 'action', actionPalette);
			//self.addSearchPalette(true);
			if(!Agnity.hasUrlReadonlyParam() || Agnity.hasLockAcquired())
				self.addSearchPalette(true);
			self.addAgnityShapesPalette(true, dir, 'generalAssets', treeGeneralAssetsShapes);
			self.addAgnityShapesPalette(true, dir, 'callAssets', treeCallAssetsShapes);
			self.addAgnityShapesPalette(true, dir, 'mediaAssets', treeMediaAssetsShapes);
			self.addAgnityShapesPalette(true, dir, 'enterpriseAssets', treeEnterpriseAssetsShapes);			
		}
		else if(Agnity.isDBSchemaDiagram())
		{
			for(var idx = 0; idx < dbSchemaGeneralAssetShapes.length; idx++)
			{
				self.licensedShapes.push(dbSchemaGeneralAssetShapes[idx]);
			}
			self.addAgnityActions(false, iconDir, 'action', componentActionPalette);
			self.addAgnityShapesPalette(true, dir, 'generalAssets', dbSchemaGeneralAssetShapes);
		}
	
		Agnity.onShapesLoaded(self.editorUi);
		
		if(Agnity.isComponentDiagram() || Agnity.isTreeDiagram())
		{
			self.createTreeHolder();
			self.showTreeData();
		}
	});
	
	
	//this.addGeneralPalette(true, dir);
 ...
 */
Sidebar.prototype.createTreeHolder = function()
{
	var self = this;
	var ui = this.editorUi;	

	var header = this.createAgnityTitle('applicationAssets', true, function(evt){
		self.showTreeData();
		mxEvent.consume(evt);
	});
	...
	this.treeHolder = document.createElement('div');
	this.treeHolder.setAttribute('class', 'AgnityTreeStructureDataTree');
	this.container.appendChild(this.treeHolder);

	var content = this.treeHolder;

	if(Agnity.isTreeDiagram())
	{
		header.classList.remove('AgnityExpandedPalette');
		header.classList.add('AgnityShrinkedPalette');	
		content.style.display = 'none';
	}
	else
	{
		header.classList.add('AgnityExpandedPalette');
		header.classList.remove('AgnityShrinkedPalette');		
	}
	
	
	mxEvent.addListener(header, 'click', mxUtils.bind(this, function(evt)
	{	
	...
	$(this.treeHolder).on('dblclick', 'a', function (event) {
		mxEvent.consume(event);

	    var node = $(event.target).closest("li");
	    var item = node[0].id;
	    
	    var currJsItem = self.jsDataLookupMap[item];
	    
	    if(currJsItem.icon == 'AgnityComponentRootNode')
	    {
			if(agnityGlobalData.tabId == null)
				window.open('TreeViewer.html?forestName=' + Agnity.getComponentData(ui).forestName + '&componentName=' + item);
			else
				agnityGlobalData.sendMessageToParent('viewComponent', {'forestName': Agnity.getComponentData(ui).forestName, 'name' : item, 'diagramType' : 'component'});

	    }
	    else if(currJsItem.icon == 'AgnityTreeRootNode')
	    {
			if(agnityGlobalData.tabId == null)
				window.open('TreeViewer.html?forestName=' + Agnity.getComponentData(ui).forestName + '&treeName=' + item);
			else
				agnityGlobalData.sendMessageToParent('connectTree', {'forestName': Agnity.getComponentData(ui).forestName, 'name' : item, 'diagramType' : 'tree'});

	    }
	});
}

Sidebar.prototype.showTreeData = function()
{		
	var self = this;
	var ui = this.editorUi;	
	...
	if(Agnity.isComponentDiagram())
	{
		...
		forestName = componentData.forestName;
	...
	if(Agnity.isTreeDiagram())
	{
		...
		forestName = treeData.forestName;
		treeName = treeData.treeName;
	}
	
	var dataProvider = new AgnityGetForestFunctionBlocks();
	dataProvider.fetchData(forestName, Agnity.getUrlParam('diagram'), Agnity.hasUrlReadonlyParam(), Agnity.getUrlParam('operationMode'), Agnity.getDomainName(), function(response)
	{
		self.jsDataLookupMap = {};
		var jsData = [];
		
		for(var idx = 0; idx < response.length; idx++)
		{
			var currNodeItem = response[idx];
			
			var currJsItem = {};
			currJsItem.id = currNodeItem.nodeName;
			currJsItem.parent = currNodeItem.parent;
			if(currJsItem.parent == '')
			{
				currJsItem.parent = '#';
			}
			currJsItem.text = currNodeItem.displayText;
			currJsItem.icon = 'AgnityTreeRootNode';
			if(currNodeItem.type == 'FunctionBlock')
			{
				currJsItem.icon = 'AgnityComponentRootNode';
				
				if(Agnity.isComponentDiagram() && currNodeItem.nodeName == componentName)
				{
					currJsItem.a_attr = {"data-agnitycurrentblock" : "1"};
				}
			}
			else
			{
				if(Agnity.isTreeDiagram() && currNodeItem.nodeName == treeName)
				{
					currJsItem.a_attr = {"data-agnitycurrentblock" : "1"};
				}
			}
			
			self.jsDataLookupMap[currJsItem.id] = currJsItem;
			
			console.log('SHAR: JSItem:' + JSON.stringify(currJsItem));

			jsData.push(currJsItem);
		}
		
		console.log('SHAR: JSData:' + JSON.stringify(jsData));
		
		$(self.treeHolder).jstree(true).settings.core.data = jsData;
		$(self.treeHolder).jstree(true).refresh();
		
		$(self.treeHolder).on('refresh.jstree', function() {
 ...
 * Adds all palettes to the sidebar.
 */
Sidebar.prototype.agnityShowTooltip = function(elt, cells, w, h, title, showLabel)
{
		...
		if (this.currentElt != elt)
		{
			...
			var show = mxUtils.bind(this, function()
			{
				...
				if (this.tooltip == null)
				{
					...
					this.graph2 = document.createElement('div');
					this.graph2.className = 'AgnitySidebarTooltip';
					this.tooltip.appendChild(this.graph2);
				...
				var hoverImageName = 'url(../themes/' + Agnity.getThemeName() + '/images/AgnityNodes/' + title.replace(new RegExp(' ', 'g'), '_').toLowerCase() + '.png)';

				this.graph2.style.backgroundImage = hoverImageName;
...
/**
 * Unique and sorted shapes array
 */
var uniqueSearchShapes = function(shapeNames)
{
	var sortedAry = [];
	$.each(shapeNames, function(i, el){
		if($.inArray(el, sortedAry) === -1) sortedAry.push(el);
	});
	...
	if (this.taglist != null && searchTerms != null)
	{
		...
		for (var i = 0; i < tmp.length; i++)
		{
			...
			if (tmp[i].length > 0)
			{
				...
				{
				   var currShape = shapesArray[i];
				   var ret = currShape.toLowerCase().search(searchKey);
				   if(ret == 0)
				   {
						searchShapes.push(currShape);
				   
				   }
				}
				searchShapes = uniqueSearchShapes(searchShapes);

				var tmpDict = new mxDictionary();
				results = [];
				for(var idx = 0; idx < searchShapes.length; idx++)
				{
					var entry = this.taglist[searchShapes[idx]];
...
Sidebar.prototype.addAgnityAction = function(action, dir)
{
	var ui = this.editorUi;
	var createBtnDiv = document.createElement('div');
	createBtnDiv.classList.add('agnity-' + action);
	if(action != 'closeToggleSidebar' && (Agnity.hasUrlReadonlyParam() || !Agnity.hasLockAcquired()))
		createBtnDiv.classList.add('AgnityDisableIcon');
	createBtnDiv.setAttribute('title', mxResources.get(action));
	
	createBtnDiv.addEventListener('click', function(event)
							  {
									ui.actions.get(action).funct();
							  });
 ...
 * Adds actions UI
 */
Sidebar.prototype.addAgnityActions = function(expand, dir, paletteName, actions)
{
	...
	div.className = 'AgnityActionsSidebar';
	
	for(var idx = 0; idx < actions.length; idx++)
	{
		div.appendChild(this.addAgnityAction(actions[idx], dir));
	}
...
Sidebar.prototype.addSearchPalette = function(expand)
{
	...
	mxEvent.addListener(input, 'input', mxUtils.bind(this, function(evt)
	{
		var inputSize = input.value.length;
		if(inputSize > 2)
		{
			find();
			mxEvent.consume(evt);
...
Sidebar.prototype.addAgnityNewShapesPalette = function(id, title, prefix, postfix, items, titles, tags)
{
	...
	for (var i = 0; i < items.length; i++)
	{
		(mxUtils.bind(this, function(item, title, tmpTags)
		{
			if (tmpTags == null)
			{
				...
				tmpTags = item.substring((slash >= 0) ? slash + 1 : 0, (dot >= 0) ? dot : item.length).replace(/[-_]/g, ' ').toLowerCase();
			}
			item = item.replace(' ', '_').toLowerCase();
			var shapeResourceName = 'shape_' + item;
			var shapeTitle = mxResources.get(shapeResourceName);
			

			fns.push(this.createVertexTemplateEntry(item + ';image;html=1;labelBackgroundColor=#ffffff;image=' + prefix + item + postfix,
				this.defaultImageWidth, this.defaultImageHeight, '', shapeTitle, title != null, null, this.filterTags(tmpTags)));
...
Sidebar.prototype.addAgnityShapesPalette = function(expand, dir, paletteName, paletteShapes)
{
	var lineTags = 'line lines connector connectors connection connections arrow arrows ';
	
	var fns = [];
	var style = ';whiteSpace=wrap;html=1;rounded=1;';
	var ignore;
	var onInit;
	var scale;
	var tags;
	...
	if (this.addStencilsToIndex)
	{
		...
		if (fns != null)	
		{
			...
			{
				customFns.push(fns[i]);
			}
		}
		
		mxStencilRegistry.loadStencilSet(dir + '/shapes.xml', mxUtils.bind(this, function(packageName, stencilName, displayName, w, h)
		{
			...
			if (ignore == null || mxUtils.indexOf(ignore, stencilName) < 0)
			{
				...
				if(Agnity.includes(this.licensedShapes, displayName) && Agnity.includes(paletteShapes, displayName))
				{
					//customFns.push(this.createVertexTemplateEntry(stencilName.toLowerCase() + ';shape=' + packageName + stencilName.toLowerCase() + style,
					//Math.round(w * scale), Math.round(h * scale), '', stencilName.replace(/_/g, ' '), null, null,
					//this.filterTags(tmp.join(' ')), true));
					var shapeResourceName = 'shape_' + stencilName.toLowerCase();
					var shapeTitle = mxResources.get(shapeResourceName);
					console.log(shapeTitle);
					console.log(shapeResourceName);
					tmp.push(shapeTitle.split(' '));
										
					customFns.push(this.createVertexTemplateEntry('label;' + stencilName.toLowerCase() + ';agnshape=' + packageName + stencilName.toLowerCase(),
					                          					Math.round(w * scale), Math.round(h * scale), '', shapeTitle, null, null,
					                          					this.filterTags(tmp.join(' ')), true));
				}
			}
		...
		if(!Agnity.hasUrlReadonlyParam() && Agnity.hasLockAcquired())
		{
			this.addAgnityPaletteFunctions(paletteName, mxResources.get(paletteName), (expand != null) ? expand : true, customFns);
		}
	}
	else
	{
		this.addPalette(paletteName, mxResources.get(paletteName), true, mxUtils.bind(this, function(content)
	    {
			if (style == null)
			{
				style = '';
			...
			mxStencilRegistry.loadStencilSet(dir + '/shapes.xml', mxUtils.bind(this, function(packageName, stencilName, displayName, w, h)
			{
				if (ignore == null || mxUtils.indexOf(ignore, stencilName) < 0)
				{
					content.appendChild(this.createVertexTemplate(stencilName.toLowerCase() + ';shape=' + packageName + stencilName.toLowerCase() + style,
						Math.round(w * scale), Math.round(h * scale), '', stencilName.replace(/_/g, ' '), true));
				}
			}), true);
	    }));
	}
	//this.addPaletteFunctions('guide', mxResources.get('guide'), (expand != null) ? expand : true, fns);
};
...
Sidebar.prototype.createAgnityTitle = function(id, refreshButtonFlag, refreshButtonHandler)
{
    var refreshButtonFlag = refreshButtonFlag || false;

	var elt = document.createElement('div');
	elt.classList.add('AgnitySidebarPalette');
	elt.classList.add('AgnitySidebarPalette_' + id);
	
	var groupName = document.createElement('div');
	groupName.classList.add('AgnitySidebarPaletteGroup');
	mxUtils.write(groupName, mxResources.get('group_' + id));
	elt.appendChild(groupName);
	
	var label = document.createElement('div');	
	label.classList.add('AgnitySidebarPaletteLabel');
	mxUtils.write(label, mxResources.get(id));
	elt.appendChild(label);

	var image = document.createElement('div');
	image.classList.add('AgnitySidebarPaletteImage');
	elt.appendChild(image);
	
	if(refreshButtonFlag)
	{
		var refreshButton = document.createElement('div');
		mxEvent.addListener(refreshButton, 'click', refreshButtonHandler);		                	
		refreshButton.classList.add('AgnitySidebarPaletteRefresh');
		elt.appendChild(refreshButton);
	}
	return elt;
};
...
Sidebar.prototype.createAgnityThumb = function(cells, width, height, parent, title, showLabel, showTitle, realWidth, realHeight)
{
	var currItem = document.createElement('div');
	currItem.setAttribute('class', 'AgnityThumbItem');
	
	var imageName = 'url(../themes/' + Agnity.getThemeName() + '/images/AgnityNodes/sidebar/' + title.replace(new RegExp(' ', 'g'), '_').toLowerCase() + '.png)';
	var hoverImageName = 'url(../themes/' + Agnity.getThemeName() + '/images/AgnityNodes/' + title.replace(new RegExp(' ', 'g'), '_').toLowerCase() + '.png)';
		
	parent.addEventListener('mouseenter', function(event)
	{
		currItem.style.backgroundImage = hoverImageName;	
	});
	
	parent.addEventListener('mouseleave', function(event)
	{
		currItem.style.backgroundImage = imageName;
	});
	
	currItem.style.backgroundImage = imageName;
	parent.appendChild(currItem);
}
...
/*Sidebar.prototype.createItem = function(cells, title, showLabel, showTitle, width, height, allowCellsInserted)
{
	...
	shapeDiv.className = 'geItem SharsbItem';
	shapeDiv.style.margin = '5px 7px 0px 0px';
	shapeDiv.style.overflow = 'hidden';
	
	var childDiv = document.createElement('div');
	childDiv.setAttribute('id', 'sb_' + title);
	childDiv.style.margin = '-25px 0px 0px 70px';
	
	var spanDiv = document.createElement('span');
	mxUtils.write(spanDiv, title);
	
	var imageDiv = document.createElement('div');
	imageDiv.className = 'gecheckMark';
	imageDiv.style.margin = '-25px 0px 0px 25px';
	
	var checkMark =document.createElement('div');
	checkMark.appendChild(imageDiv);
	
	var checkStem =document.createElement('div');
	checkMark.appendChild(checkStem);
	// Blocks default click action
	mxEvent.addListener(shapeDiv, 'click', function(evt)
	{
		mxEvent.consume(evt);
	});

	this.createThumb(cells, this.thumbWidth, this.thumbHeight, shapeDiv, title, showLabel, showTitle, width, height);
	var bounds = new mxRectangle(0, 0, width, height);
	
	if (cells.length > 1 || cells[0].vertex)
	{
		var ds = this.createDragSource(shapeDiv, this.createDropHandler(cells, true, allowCellsInserted,
			bounds), this.createDragPreview(width, height), cells, bounds);
		this.addClickHandler(shapeDiv, ds, cells);
	...
	{
		var ds = this.createDragSource(shapeDiv, this.createDropHandler(cells, false, allowCellsInserted,
			bounds), this.createDragPreview(width, height), cells, bounds);
		this.addClickHandler(shapeDiv, ds, cells);
	}
	...
	if (!mxClient.IS_IOS)
	{
		mxEvent.addGestureListeners(shapeDiv, null, mxUtils.bind(this, function(evt)
		{
			if (mxEvent.isMouseEvent(evt))
			{
				this.showTooltip(shapeDiv, cells, bounds.width, bounds.height, title, showLabel);
			}
		}));
	}
	
	//elt.style.height = '85px';
	
	
	//parent.appendChild(elt);
	
	childDiv.appendChild(spanDiv);
	shapeDiv.appendChild(childDiv);
...
Sidebar.prototype.createItem = function(cells, title, showLabel, showTitle, width, height, allowCellsInserted)
{
	var elt = document.createElement('a');
	elt.setAttribute('href', 'javascript:void(0);');
	elt.className = 'geItem AgnityThumbHolder';
	...
	this.createAgnityThumb(cells, this.thumbWidth, this.thumbHeight, elt, title, showLabel, showTitle, width, height);
	var bounds = new mxRectangle(0, 0, width, height);
	...
	if (!mxClient.IS_IOS)
	{
		mxEvent.addGestureListeners(elt, null, mxUtils.bind(this, function(evt)
		{
			if (mxEvent.isMouseEvent(evt))
			{
				this.agnityShowTooltip(elt, cells, bounds.width, bounds.height, title, showLabel);
			}
		}));
	}
...
{
	console.log('SHAR: Style is ' + style);
...
Sidebar.prototype.addAgnityPaletteFunctions = function(id, title, expanded, fns)
{
	this.addPalette(id, title, expanded, mxUtils.bind(this, function(content)
	{
		for (var i = 0; i < fns.length; i++)
		{
			content.classList.add("AgnitySidebarThumbHolder");
			content.appendChild(fns[i](content));
		}
	}));
};
...
{
	var elt = this.createAgnityTitle(id);
	this.container.appendChild(elt);
	...
	{
		onInit(div);
		elt.classList.add('AgnityExpandedPalette');
		onInit = null;
	}
	else
	{
		elt.classList.add('AgnityShrinkedPalette');
		div.style.display = 'none';
	}
...
Sidebar.prototype.addFoldingHandler = function(title, content, funct)
{
	...
	mxEvent.addListener(title, 'click', mxUtils.bind(this, function(evt)
	{
		if (content.style.display == 'none')
		{
			...
			title.classList.add('AgnityExpandedPalette');
			title.classList.remove('AgnityShrinkedPalette');
		//	title.style.backgroundImage = 'url(\'' + this.expandedImage + '\')';
		}
		else
		{
			title.classList.remove('AgnityExpandedPalette');
			title.classList.add('AgnityShrinkedPalette');
